...
# run parameters
event_list = [99]
#matchingTime_list = np.concatenate((np.linspace(0.4,2.0,9), np.linspace(3,10,8)),1)
matchingTime_list = np.linspace(3,10,8)
# folders locations
rootDir = path.abspath('..')  #root directory for current node
database_location = path.join(rootDir,'localdataBase') # 
...
MCevents_location = path.join(rootDir, 'superMC','data')
...
def meanPTCalculatorShell():
    ...
    meanPT_file = path.join(database_location, 'meanPT.dat')
    meanPT_log = open(meanPT_file, 'w')
    for event_num in event_list:
        particle_file = path.join(MCevents_location,'sd_event_%d_block_particle.dat'%event_num)
        energy_file   = path.join(MCevents_location,'sd_event_%d_block.dat'%event_num)
        ...
        for tau_s in matchingTime_list:
            ...
            # get photon pT
            is_data_folder = path.join(database_location, 'event_%d'%event_num,'%g'%tau_s)
