...
taus_index = int(node_index.split('e')[-1]) # index of tau_s for current folder
...
def runHydro(mth_time, hydro_dir, inital_phi, etas, edec):
    """
...
for eta_s in eta_s_list:
	for tdec_idx in range(len(tdec_list)):
		...
		while 1:
			...
			runHydro(matching_time, runcode.hydroDirectory, 0.0, 
				eta_s, edec)
			...
			if( abs(totaldEdyTest-totaldEdyExpect) < 10):
				sfactor_log.write("%6.2f       %20.8f    %20.8f		%20.8f    %20.4f\n"   \
					%(matching_time, eta_s, tdec, runcode.norm_factor, totaldEdyTest))
